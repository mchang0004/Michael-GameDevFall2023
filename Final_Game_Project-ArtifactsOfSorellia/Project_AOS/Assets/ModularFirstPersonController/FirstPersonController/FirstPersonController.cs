...
public class FirstPersonController : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (enableCrouch)
        {
            if(Input.GetKeyDown(crouchKey) && !holdToCrouch && !Input.GetKey(sprintKey))
			{
                Crouch();
            }
            ...
            else if(Input.GetKeyUp(crouchKey) && holdToCrouch && !Input.GetKey(sprintKey))
            {
                isCrouched = true;
                Crouch();
            }
	...
	private IEnumerator StartJumpCooldown(float cooldownTime)
	{
		jumpCooldown = true;
